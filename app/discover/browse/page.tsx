...
export default async function BrowseCatalog({ searchParams }: { searchParams: { url: string } }) {
                    ...
                    <div className="mt-auto space-y-2">
                       {epubLink && (
                         <Button size="sm" className="w-full text-xs gap-1" asChild>
                            <Link href={`/api/import-opds?epubUrl=${encodeURIComponent(epubLink.href)}&title=${encodeURIComponent(entry.title)}`}>
