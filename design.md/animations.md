...
To keep motion unified across the app:

- Define durations/easings as CSS variables in `app/globals.css`
- Reuse shared motion classes (for example `motion-button`, `motion-card`, `motion-page-enter`)
- Do not hardcode animation values in component TSX unless there is no tokenized alternative
- Keep paired elements on the same duration/easing (dialog content + overlay, popover + arrow/container)

## OpenBookshelf Motion Mapping

- Navigation links and high-frequency controls use `motion-nav-link` only (micro timing).
- Card hover interactions (library items, modules, settings toggles) use `motion-card` and only on fine pointers.
- Reader session controls keep motion minimal because they are used repeatedly.
- Onboarding and settings surfaces can use standard enter animation because they are low-frequency.
